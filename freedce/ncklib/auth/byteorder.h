...
#define DBG_RW_PCVAL(charmode,string,depth,base,read,inbuf,outbuf,len) \
	...
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%s%04x %s: ", \
             tab_depth(depth), base,string)); \
    if (charmode) print_asc(depth+5, (unsigned char*)(outbuf), (len)); else \
	{ uint32 idx; for (idx = 0; idx < (uint32)len; idx++) { RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%02x ", (outbuf)[idx])); } } \
...
#define DBG_RW_PSVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
	...
	{ uint32 idx; for (idx = 0; idx < (uint32)len; idx++) { RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%04x ", (outbuf)[idx])); } } \
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("\n")); }
...
#define DBG_RW_PIVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
	...
	{ uint32 idx; for (idx = 0; idx < (uint32)len; idx++) { RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%08x ", (outbuf)[idx])); } } \
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("\n")); }

#define DBG_RW_CVAL(string,depth,base,read,inbuf,outbuf) \
	{ RW_CVAL(read,inbuf,outbuf,0) \
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%s%04x %s: %02x\n", \
             tab_depth(depth), base, string, outbuf)); }

#define DBG_RW_SVAL(string,depth,base,read,big_endian,inbuf,outbuf) \
	{ RW_SVAL(read,big_endian,inbuf,outbuf,0) \
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%s%04x %s: %04x\n", \
             tab_depth(depth), base, string, outbuf)); }

#define DBG_RW_IVAL(string,depth,base,read,big_endian,inbuf,outbuf) \
	{ RW_IVAL(read,big_endian,inbuf,outbuf,0) \
	RPC_DBG_ADD_PRINTF(rpc_e_dbg_auth, 5+depth,("%s%04x %s: %08x\n", \
             tab_depth(depth), base, string, outbuf)); }
