...
error_status_t  *status;
{
    ...
    while(true) 
    {
        dsm_read(h->dsh,&marker,(void **) &entp,status);    /* get next record */
        if (! STATUS_OK(status))
        {
...
rpc_protocol_id_t       rpc_protocol;
unsigned32              rpc_protocol_vers_major;
unsigned32              rpc_protocol_vers_minor __attribute__((__unused__));
rpc_protseq_id_t        protseq;
unsigned32              max_ents;
unsigned32              *n_ents;
db_entry_t              *entries[];
ept_lookup_handle_t     *map_handle;
unsigned32              pass __attribute__((__unused__));
db_list_type_t          list_type;
db_lists_t              **lpp;
unsigned32              *status;
{
