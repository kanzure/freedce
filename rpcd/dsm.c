...
error_status_t *st;         /* (output) status */
{
    ...
    if (fname != NULL && ustrlen(fname) != 0) {
        ...
        if (write(fd,&fhdr,PAGE_SIZE) != PAGE_SIZE) SIGNAL(dsm_err_file_io_error); /* write hdr */
    }
...
error_status_t *st;         /* (output) status */
{
    ...
    if (fname != NULL && ustrlen(fname) != 0) {
        ...
        if (read(fd,&fhdr,PAGE_SIZE) != PAGE_SIZE) SIGNAL(dsm_err_file_io_error); /* read hdr */
    ...
    if (dsh->pages > 0) {           /* if any valid pages */
        ...
        */
        if ((unsigned32)read(dsh->fd,dsh->map->ptr,dsh->map->size) != dsh->map->size) { /* read in the file */
...
error_status_t *st;    /* status */
{
    ...
    */
    ...
    ||  read(dsh->fd,&hdr,PAGE_SIZE) != PAGE_SIZE) SIGNAL(dsm_err_file_io_error);
...
dsm_handle_t   dsx __attribute__((__unused__));
error_status_t *st;
{
#ifndef DSM_DEBUG
...
error_status_t *st;
{
    ...
    */
    ...
    if (dsh->fd >= 0) { /* only if there's a file, */
        if ((unsigned32)write(dsh->fd,p,grow_bytes) != grow_bytes) SIGNAL(dsm_err_file_io_error);
    ...
    */
    ...
    for (map = dsh->map; map != NULL; map = map->link) {    /* for each file chunk */
        ...
        */
        ...
        for (;;) {                                      /* until explicit break at end of chunk */
            next = (block_t *) (((char *)this)+this->size+PREHEADER);   /* find next block */
            if ((unsigned32)((char *)next-(char *)map->ptr) >= map->size) {   /* is this the last block? */
...
dsm_handle   dsh;  /* data store handle */
{
    ...
    for (map = dsh->map; map != NULL; map = map->link) {    /* for each file chunk */
        ...
        for (;;) {                                      /* until explicit break at end of chunk */
            ...
            next = (block_t *) (((char *)this)+this->size+PREHEADER);   /* find next block */
            if ((unsigned32)((char *)next-(char *)map->ptr) >= map->size) break;   /* is this the last block? */
...
dsm_handle     dsh;
dsm_marker_t   marker __attribute__((__unused__));
{
