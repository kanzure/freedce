...
type called USHORT that is in the local machines byte order, and you
want to do it with only the assumption that USHORT is _at_least_ 16
...
then to extract a USHORT value at offset 25 in a buffer you do this:

char *buffer = foo_bar();
USHORT xx = SVAL(buffer,25);
...
#define SVALS(buf,pos) ((SHORT)SVAL(buf,pos))
#define IVALS(buf,pos) ((LONG)IVAL(buf,pos))
#define SSVAL(buf,pos,val) SSVALX((buf),(pos),((USHORT)(val)))
#define SIVAL(buf,pos,val) SIVALX((buf),(pos),((ULONG)(val)))
#define SSVALS(buf,pos,val) SSVALX((buf),(pos),((SHORT)(val)))
#define SIVALS(buf,pos,val) SIVALX((buf),(pos),((LONG)(val)))

#define SVALMOD(buf,pos) (PVAL(buf,pos)|PVAL(buf,(pos)+1)<<8)
#define IVALMOD(buf,pos) (SVALMOD(buf,pos)|SVALMOD(buf,(pos)+2)<<16)
#define SVALSMOD(buf,pos) ((SHORT)SVALMOD(buf,pos))
#define IVALSMOD(buf,pos) ((LONG)IVALMOD(buf,pos))
...
/* this handles things for architectures like the 386 that can handle
   alignment errors */
/*
   WARNING: This section is dependent on the length of SHORT and LONG
   being correct 
*/

/* get single value from an SMB buffer */
#define SVAL(buf,pos) (*(const USHORT *)((const char *)(buf) + (pos)))
#define IVAL(buf,pos) (*(const ULONG *)((const char *)(buf) + (pos)))
#define SVALS(buf,pos) (*(const SHORT *)((const char *)(buf) + (pos)))
#define IVALS(buf,pos) (*(const LONG *)((const char *)(buf) + (pos)))

/* store single value in an SMB buffer */
#define SVALMOD(buf,pos) (*(USHORT *)((char *)(buf) + (pos)))
#define IVALMOD(buf,pos) (*(ULONG *)((char *)(buf) + (pos)))
#define SVALSMOD(buf,pos) (*(SHORT *)((char *)(buf) + (pos)))
#define IVALSMOD(buf,pos) (*(LONG *)((char *)(buf) + (pos)))

#define SSVAL(buf,pos,val) SVALMOD(buf,pos)=((USHORT)(val))
#define SIVAL(buf,pos,val) IVALMOD(buf,pos)=((ULONG)(val))
#define SSVALS(buf,pos,val) SVALSMOD(buf,pos)=((SHORT)(val))
#define SIVALS(buf,pos,val) IVALSMOD(buf,pos)=((LONG)(val))
...
{ ULONG l; for (l = 0; l < (ULONG)(len); l++) (val)[l] = macro((buf), (pos) + (size)*l); }

#define SSMBMACRO(macro,buf,pos,val,len,size) \
{ ULONG l; for (l = 0; l < (ULONG)(len); l++) macro((buf), (pos) + (size)*l, (val)[l]); }
...
#define DBG_RW_PCVAL(charmode,string,depth,base,read,inbuf,outbuf,len) \
	...
	{ ULONG idx; for (idx = 0; idx < len; idx++) { DEBUG(5+depth,("%02x ", (outbuf)[idx])); } } \
	DEBUG(5+depth,("\n")); } 
...
#define DBG_RW_PSVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
	...
	{ ULONG idx; for (idx = 0; idx < len; idx++) { DEBUG(5+depth,("%04x ", (outbuf)[idx])); } } \
	DEBUG(5+depth,("\n")); }
...
#define DBG_RW_PIVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
	...
	{ ULONG idx; for (idx = 0; idx < len; idx++) { DEBUG(5+depth,("%08x ", (outbuf)[idx])); } } \
	DEBUG(5+depth,("\n")); }
